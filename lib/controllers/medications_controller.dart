...
Future<void> deleteMedication(Medication med) async {
  ...
  if (isOnline) {
    try {
      ...
      // لو فشل الحذف من Supabase، اعمل soft delete
      await database.medicationsDao.markAsDeleted(med.medId!, 'not_synced');
    }
  } else {
    // لو مفيش نت: اعمل soft delete (mark as deleted)
    await database.medicationsDao.markAsDeleted(med.medId!, 'not_synced');
  }
    ...
    try {
      ...
      if (isOnline) {
        try {
          ...
          print('⚠️ Failed to delete old schedules from Supabase: $e');
        }
      }
      ...
      if (isOnline && savedSchedules.isNotEmpty) {
        try {
          ...
          print('⚠️ Failed to sync new schedules: $e');
        }
      }
